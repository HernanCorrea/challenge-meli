...
import { catchError, debounceTime, map, mergeMap, of, OperatorFunction, tap, throttleTime, withLatestFrom } from "rxjs";
import { ItemDetailJSON } from "../../interfaces";
import { ItemService } from "../../services/item.service";
import { AppState } from "../app.state";
import { getCurrentRoute } from "../router/router.selector";
import { getItem, searchItems, setDetail, setItems } from "./item.actions";
    ...
    this.actions$
      .pipe(
        ...
        mergeMap(([action, currentRoute]) => {
          console.log({action, currentRoute});
            const search = action?.query ?? currentRoute.queryParams['search'];
            return this.itemService.getItems(search)
              .pipe(
                map((items: any) => {
